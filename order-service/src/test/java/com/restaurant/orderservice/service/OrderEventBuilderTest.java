 ...
 * 
 * Tests domain event construction from domain entities.
 */
class OrderEventBuilderTest {
    ...
    void buildOrderPlacedEvent_withSingleItem_createsCorrectEvent() {
        ...
        assertThat(event.getEventType()).isEqualTo(OrderPlacedDomainEvent.EVENT_TYPE);
        assertThat(event.getEventVersion()).isEqualTo(OrderPlacedDomainEvent.CURRENT_VERSION);
    ...
    void buildOrderPlacedEvent_doesNotIncludeItemNotes() {
        ...
        assertThat(eventItem.getProductId()).isEqualTo(10L);
        assertThat(eventItem.getQuantity()).isEqualTo(2);
        // OrderItemData only has productId and quantity, no note field
    }
